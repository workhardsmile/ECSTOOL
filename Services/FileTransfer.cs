...
namespace ECSTOOL
{
    ...
    public abstract class FileTransmission : IDisposable
    {
        ...
        internal virtual void OnConnectLost()
        {
            ...
            Stop();
            if (ConnectLost != null)
                ConnectLost((IPEndPoint)this._Socket.RemoteEndPoint, new EventArgs());
        }
        /// <summary>
        /// 同步发送字符串
        /// </summary>
    ...
    public class FileSender : FileTransmission
    {
        ...
        internal bool OnCommandReceived(string str)
        {
            ...
            else if (Msg[0] == "Complete" || Msg[0] == "NextFile")
            {
                //Stop();
                //OnAllFinished();
    ...
    public class SendSocket
    {
        ...
        public int QueueErrosCount;
        public int QueueMessagesCount;
        public int MaxQueueLength = 100;
        public Queue<string> QueueErros;
        public Queue<string> QueueMessages;

        public static bool IsFinished = true;
        public Queue<MessagePackage> MessageQueue;  //待处理数据包队列（主线程）

        public SendSocket(int _socketPort, string[] _fullFileName)
        {
            this.Port = _socketPort;
            this.FullFileName = _fullFileName;
            QueueErros = new Queue<string>(MaxQueueLength);
            QueueMessages = new Queue<string>(MaxQueueLength);
        ...
        public void BeginListen()
        {
            try
            {
                ...
                while (listensocket != null)
                {
                    ...
                    task = new FileSender();
                    if (this.Index < this.FullFileName.Length)
                    {
                        task.FullFileName = FullFileName[this.Index];
                    }
                    else
                    {
                        this.Index = 0;
                        task.FullFileName = FullFileName[0];
                    }
        ...
        public void BeginNewFile()
        {
            try
            {
                if (task != null && this.Index < this.FullFileName.Length)
                {
                    this.Index++;
                }
                else
        ...
        {
            if (MaxQueueLength > QueueErrosCount)
            {
                Interlocked.Increment(ref QueueErrosCount);
                lock (QueueErros)
                {
                    QueueErros.Enqueue(e.InnerException.ToString());  // 数据包入队列
                }
            }
            else
            {
                string data = QueueErros.Dequeue();  // 取队列数据
                data = null;
                lock (QueueErros)
                {
                    QueueErros.Enqueue(e.InnerException.ToString());  // 数据包入队列
        ...
        void task_BlockFinished(object sender, BlockFinishedEventArgs e)
        {
            ...
            this.Sended = string.Format("已发送:{0:N2}%", task.Progress);
            this.Speed = string.Format("{0:N2}KB/s", task.KByteAverSpeed);
        }
        void task_ConnectLost(object sender, EventArgs e)
        {
            if (MaxQueueLength > QueueErrosCount)
            {
                ...
                lock (QueueErros)
                {
                    QueueErros.Enqueue(((IPEndPoint)sender).Address.ToString() + " ConnectLost");  // 数据包入队列
                }
            }
            else
            {
        ...
        {
            if (MaxQueueLength > QueueMessagesCount)
            {
                Interlocked.Increment(ref QueueMessagesCount);
                lock (QueueMessages)
                {
                    QueueMessages.Enqueue(e.CommandLine);  // 数据包入队列
                }
            }
            else
            {
                string data = QueueMessages.Dequeue();  // 取队列数据
                data = null;
                lock (QueueErros)
                {
                    QueueMessages.Enqueue(e.CommandLine);  // 数据包入队列
            ...
            if (e.CommandLine == "NextFile")
            {
                BeginNewFile();
            }
            else if (e.CommandLine == "Exit")
            {
                this.FinshCount++;
                if (this.FinshCount >= this.PkgCount)
                {
                    IsFinished = true;
                }
            }
            //Console.WriteLine(e.CommandLine);
        }
    }
    #endregion
    ...
    public class ReceiveSocket
    {
        ...
        public ReceiveSocket(string _ip, int _port, string[] _dirPath, int _fileCount)
        {
            this.IP = _ip;
            this.Port = _port;
        ...
        public void BeginReceive()
        {
            ...
            task.ErrorOccurred += new FileTransmissionErrorOccurEventHandler(task_ErrorOccurred);
            if (this.Index < this.DirPath.Length)
            {
                task.FilePath = this.DirPath[Index];
            }
            else
            {
                this.Index = 0;
        ...
        public void BeginNewFile()
        {
            if (this.count < this.fileCount)
            {
                ...
                else
                {
                    ...
                    if (this.count < this.fileCount)
                    {
                        task.SendStringAsync("NextFile");
                    }
                    else
                    {
                        if (!IsFinished)
                        {
                            task.SendStringAsync("Exit");
                            IsFinished = true;
                            //task.Stop();
                            //this.Close();
                        }
        ...
        void task_ErrorOccurred(object sender, FileTransmissionErrorOccurEventArgs e)
        {
            //this.Error = "异常：" + e.InnerException.ToString();
            //task.SendStringAsync("Exit");
            IsFinished = true;
            MessageType.AddMessageQueue(ref this.MessageQueue, LogFile.Error+"|[" + DateTime.Now.ToLongTimeString() + "]异常：" + e.InnerException.ToString(), MessageType.OUTPUT_LOG, Program.IP);
        }
        ...
        {
            FileTransmission task = (FileTransmission)sender;
            this.Maximum = task.Blocks.Count;
            this.CurrentValue = task.Blocks.CountValid;
            SetProgress(task);
        }
        void task_BlockHashed(object sender, BlockFinishedEventArgs e)
        {
            FileTransmission task = (FileTransmission)sender;
            this.Message = "接收端 校验中";
        }
        void SetProgress(FileTransmission task)
        {
            this.Message = "接收端 下载中";
            this.Result = string.Format("进度:{0:N2}%   总长度:{1}   已完成:{2}", task.Progress, task.TotalSize, task.FinishedSize) + "\r\n";
            //this.Result += string.Format("平均速度:{0:N2}KB/s   已用时:{1}   估计剩余时间:{2}", task.KByteAverSpeed, task.TimePast, task.TimeRemaining) + "\n";
            this.Result += string.Format("平均速度:{0:N2}KB/s   已用时:{1}", task.KByteAverSpeed, task.TimePast) + "\r\n";
            this.Result += string.Format("瞬时速度:{0:N2}KB/s   丢弃的区块:{1}", task.KByteSpeed, task.Blocks.Cast.Count) + "\r\n";
