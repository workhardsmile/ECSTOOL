...
namespace ECSTOOL
{
    ...
    public delegate void DelegateCheckSession(object sender, EventArgs e);

    #region  数据包接收器类 ClientReceiver
    ...
    public class ClientReceiver
    {
        ...
        private int _maxSocketDataTimeout = 180;             // 最大无数据接收时间（180秒）
        ...
        public Hashtable _hostTable;
        public Hashtable _hostToken;
        ...
        public Hashtable ClientSessionTable  // 当前在线会话列表副本
        {
            get
            {
                ...
                lock (_sessionTable)
                {
                    ...
                    {
                        sessionOnline.Add(session.IP, session);
                    }
                }
        ...
        {
            _hostTable = new Hashtable();
            _hostToken = new Hashtable();
            //_hostTable.Add(_hostIP, _tcpSocketPort);
        }  // 使用默认参数
        ...
        public bool StartAllReceiver()
        {
            try
            {
                ...
                //{
                //    this.ConnectThread = new Thread(ConnectAllServer);
        ...
        public bool ConnectOneServer(string _ip)
        {
            ...
            try
            {
                ...
                IPEndPoint remoteEP = new IPEndPoint(ipAddress, (int)_hostTable[_ip]);
        ...
        private static void ConnectCallback(IAsyncResult ar)
        {
            ...
            {
                MessageType.MessageBoxShow(e.Message, "error");
            }
        }
        /// <summary>
        ...
        public void ConnectAllServer(object state)
        {
            ...
            Socket client = null;
            //string[] _ips = _hostTable.Keys;

            foreach (string _ip in _hostTable.Keys)
            {
                try
                {
                    IPHostEntry ipHostInfo = Dns.Resolve(_ip);
                ...
                foreach (TSession session in sessionTable2.Values)  // 对副本进行操作
                {
                    ...
                    try
                    {
                        ...
                        else if (session.State == TSessionState.Closed)
                        {
                            ...
                            DelegateCheckSessionF(session.IP + ":closed", null);
                        }
                        else if (session.State == TSessionState.Normal)  // 正常的会话,发送心跳 
                        {
                            if (count >= _maxSocketDataTimeout)
                            {
                                TDatagram datagram = new TDatagram("Token: " + (string)_hostToken[session.IP]);
                                datagram.IP = session.IP;
                                lock (_datagramQueueResponse)
                                {
                                    _datagramQueueResponse.Enqueue(datagram);
                                }
                            }
                            DelegateCheckSessionF(session.IP + ":normal", null);
        ...
        private void HandleOneDatagram()
        {
            ...
            lock (_datagramQueueRequest)
            {
                if (_datagramQueueRequest.Count > 0)
                {
                    ...
                    {
                        if (datagram.Datagram != null && datagram.ResolveDatagram())
                        {
                            if (datagram.Datagram.IndexOf("Token:") >= 0)
                            {
                                string message = LogFile.Connect + "|[" + DateTime.Now.ToLongTimeString() + "] Response From <" + datagram.IP + ">! " + datagram.Datagram;
                                MessageType.AddMessageQueue(ref MessageQueue, message, MessageType.OUTPUT_LOG, Program.IP);
                            }
                            else if (datagram.Datagram.IndexOf("MessagePackage|") >= 0)
                            {
            ...
            while (!_stopReceiver)
            {
                ...
                lock (_datagramQueueResponse)
                {
                    if (_datagramQueueResponse.Count > 0)
                    {
                        ...
                        //Interlocked.Decrement(ref _datagramQueueResponseCount);
                        if (_sessionTable.ContainsKey(datagram.IP))
                        {
                            if (datagram.Datagram.IndexOf("Token:") >= 0)
                            {
                                string message = LogFile.Connect + "|[" + DateTime.Now.ToLongTimeString() + "] Request To <" + datagram.IP + ">! " + datagram.Datagram;
                                MessageType.AddMessageQueue(ref MessageQueue, message, MessageType.OUTPUT_LOG, Program.IP);
                            }
                            string s = Encoding.Default.GetString(datagram.EncryptDatagram());
                            SendData(datagram.IP, datagram.EncryptDatagram());
                            //SendData(datagram.IP, Encoding.Default.GetBytes(datagram.Datagram));
                        }
                    }
                }
